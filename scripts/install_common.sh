#!/bin/sh
# Common install driver shared by app/tool install.sh scripts.
#
# An install.sh that uses this only needs:
#     #!/bin/sh
#     . "$(cd "$(dirname "$0")"; pwd)/../../scripts/install_common.sh"
#
# It generates config.txt (build-time tunables captured from the current
# environment), resolves the build mode and prefix, runs setup.sh and the
# config/<mode>/{preprocess,build,install,postprocess}.sh steps, and writes
# the versioned <name>vars-<version>-<revision>.sh file. version.sh in the
# caller's directory must define __NAME__/__VERSION__/__MA_REVISION__.
#
# The tunables below are a superset of what individual apps used; a variable
# an app's config/ scripts do not reference is simply exported unused.
