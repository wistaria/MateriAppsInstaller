...
詳細なビルドログは ``$BUILD_DIR/<name>-<version>-<revision>.log`` （デフォルトでは ``$HOME/build`` 以下）に保存されており、ビルドコマンドが失敗した場合は ``See log for details: <パス>`` としてそのパスが表示されます。
``Error: <ディレクトリ> exists`` でインストールが停止した場合は、同じバージョンがインストール済みです。そのディレクトリを削除するか、 ``version.sh`` 内の ``<NAME>_MA_REVISION`` の数値を上げて新しいリビジョンとしてインストールしてください。
