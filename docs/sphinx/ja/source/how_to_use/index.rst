...
- ネットワーク環境に関する注意
  ...
  - ソースアーカイブのダウンロードはすべて HTTPS で行われる。プロキシサーバ経由でビルドする場合は、 ``http_proxy`` だけでなく ``https_proxy`` を環境変数に設定すること。
  - TLS 証明書エラーでダウンロードに失敗する場合は、システムの CA 証明書（ ``ca-certificates`` パッケージ等）を更新すること。
  - ネットワークに接続できない計算機では、別の計算機でダウンロードしたソースアーカイブをあらかじめ ``SOURCE_DIR`` （デフォルト: ``$HOME/source`` ）に置いておけば、ダウンロードはスキップされる。
