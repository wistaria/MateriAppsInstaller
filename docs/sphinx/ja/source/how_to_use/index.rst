...
``ma.sh`` を用いたクイックインストール
-----------------------------------------

- ツールの依存関係を宣言しているアプリについては、1つのコマンドで不足しているツールを依存順にビルドした後、アプリ本体をビルドできる。 ::

    sh ma.sh install hphi        # 必要に応じて cmake/openmpi/scalapack をビルドした後、HPhi をビルド
    sh ma.sh list                # 利用可能なアプリ・ツールの一覧 ([deps] は自動でツールを解決)
    sh ma.sh installed           # 既にインストール済みのもの

  - 自動的な依存関係の解決は、 ``sh ma.sh list`` で ``[deps]`` と表示されるアプリにのみ適用される。それ以外のアプリでは、 ``ma.sh`` はそのアプリ自体のみをインストールするため、事前に必要なツールを（以下の各ディレクトリごとの手順で）インストールしておく必要がある。
  - ``ma.sh`` はツールをデフォルトモードでビルドする。

- 以下では、 ``ma.sh`` が内部で行っている、各ディレクトリごとの詳細なインストール手順を説明する。
