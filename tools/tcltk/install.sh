...
for m in tcl tk; do
  for process in preprocess build install postprocess; do
    if [ -f $CONFIG_DIR/${process}-${m}.sh ]; then
      echo "[${process}-${m}]" | tee -a $LOG
      pipefail check sh $SHFLAG $CONFIG_DIR/${process}-${m}.sh \| tee -a $LOG
    elif [ -f $DEFAULT_CONFIG_DIR/${process}-${m}.sh ]; then
      echo "[${process}-${m}]" | tee -a $LOG
