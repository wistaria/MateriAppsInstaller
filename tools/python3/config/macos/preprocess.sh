...
if [ -n "$TCLTK_ROOT" ]; then
  ...
  ./configure CPPFLAGS="$CPPFLAGS" LDFLAGS="$LDFLAGS" --prefix=$PREFIX --enable-shared --with-system-ffi $OPENSSL_FLAGS --with-tcltk-includes="-I${TCLTK_ROOT}/include" --with-tcltk-libs="-L${TCLTK_ROOT}/lib $TCL_LIB $TK_LIB"
else
  ./configure CPPFLAGS="$CPPFLAGS" LDFLAGS="$LDFLAGS" --prefix=$PREFIX --enable-shared --with-system-ffi $OPENSSL_FLAGS
fi
