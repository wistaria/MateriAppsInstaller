...
echo "[make]" | tee -a $LOG
check ./configure --prefix=$PREFIX --enable-shared --enable-threads --enable-avx | tee -a $LOG
check make -j4 | tee -a $LOG
echo "[make install]" | tee -a $LOG
make install | tee -a $LOG
check make clean | tee -a $LOG
echo "[make float version]" | tee -a $LOG
check ./configure --prefix=$PREFIX --enable-shared --enable-threads --enable-avx --enable-float | tee -a $LOG
check make -j4 | tee -a $LOG
echo "[make install]" | tee -a $LOG
make install | tee -a $LOG
