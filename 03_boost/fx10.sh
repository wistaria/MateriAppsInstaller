...
$SUDO_TOOL rm -rf boost_$BOOST_VERSION boost_$BOOST_FCC_VERSION
if [ -f $HOME/source/boost_$BOOST_VERSION.tar.bz2 ]; then
  ...
  check wget -O - http://sourceforge.net/projects/boost/files/boost/$BOOST_VERSION/boost_$BOOST_VERSION.tar.bz2/download | $SUDO_TOOL tar jxf -
fi

$SUDO_TOOL mv -f boost_$BOOST_VERSION boost_$BOOST_FCC_VERSION
cd boost_$BOOST_FCC_VERSION
cat $SCRIPT_DIR/boost_$BOOST_FCC_VERSION.patch | $SUDO_TOOL patch -p1

cat << EOF > $BUILD_DIR/boostvars.sh
export BOOST_ROOT=$PREFIX_TOOL/boost/boost_$BOOST_FCC_VERSION
EOF
BOOSTVARS_SH=$PREFIX_TOOL/boost/boostvars-$BOOST_FCC_VERSION.sh
