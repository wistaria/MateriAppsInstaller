...
print(dirs_apps)
cwd_path = os.getcwd()
path_to_sphinx_ja = os.path.join(cwd_path, path_to_sphinx_ja)
...
for dir_name in dirs_apps:
    ...
    if os.path.isfile(path_to_README):
        ...
        cmd = "pandoc -f markdown -t rst -o README_{}.rst README_ja.md".format(dir_name[2:])
        subprocess.call(cmd.split())
        cmd = "mv README_{}.rst {}".format(dir_name[2:], path_to_sphinx_ja)
        subprocess.call(cmd.split())
        os.chdir(cwd_path)

#make index.rst in sphinx dir

with open(os.path.join(path_to_sphinx_ja, "index.rst"), "w") as fw:
