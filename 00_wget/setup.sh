...
if [ -d wget-$WGET_VERSION ]; then :; else
  if [ -f $HOME/source/wget-$WGET_VERSION.tar.gz ]; then
    check tar zxf $HOME/source/wget-$WGET_VERSION.tar.gz
  else 
...
if [ -d nettle-$NETTLE_VERSION ]; then :; else
  if [ -f $HOME/source/nettle-$NETTLE_VERSION.tar.gz ]; then
    check tar zxf $HOME/source/nettle-$NETTLE_VERSION.tar.gz
  else 
...
if [ -d gnutls-$GNUTLS_VERSION ]; then :; else
  if [ -f $HOME/source/gnutls-$GNUTLS_VERSION.tar.xz ]; then
    check tar Jxf $HOME/source/gnutls-$GNUTLS_VERSION.tar.xz
  else 
